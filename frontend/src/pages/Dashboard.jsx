  ...
  if (active && payload && payload.length) {
    return (
      <Box sx={{ bgcolor: 'background.paper', border: '1px solid #e2e8f0', borderRadius: 1, p: 1.5, boxShadow: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary">{Number(payload[0].value).toLocaleString()} {t('currency')}</Typography>
      </Box>
    )
  }
  return null
            ...
            <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="h5" fontWeight={700} color={key === 'balance' ? (data.balance >= 0 ? 'primary.main' : 'error.main') : color}>
                  {Number(data[key]).toLocaleString()} {t('currency')}
                </Typography>
              </Box>
            </CardContent>
          </Card>
                ...
                <TableBody>
                  {data.recent_transactions.map((tx) => (
                    <TableRow key={tx.id}>
                      ...
                      <TableCell align="right" sx={{ fontWeight: 600 }}>{tx.amount.toLocaleString()} {t('currency')}</TableCell>
                      <TableCell sx={{ display: { xs: 'none', sm: 'table-cell' } }}>{tx.description || '-'}</TableCell>
                    </TableRow>
                  ))}
